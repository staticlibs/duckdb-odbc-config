...
pub(super) struct AddDsnDialogNui {
    inner: Rc<RefCell<AddDsnDialog>>,
    inner_events: Rc<AddDsnDialogEvents>,
    default_handler: RefCell<Option<nwg::EventHandler>>
}

impl AddDsnDialogNui {
    pub(super) fn result(&mut self) -> AddDsnDialogResult {
        self.inner.borrow_mut().result()
    }
}

impl nwg::NativeUi<AddDsnDialogNui> for AddDsnDialog {
    fn build_ui(mut dialog: AddDsnDialog) -> Result<AddDsnDialogNui, nwg::NwgError> {
        let mut events: AddDsnDialogEvents = Default::default();
        dialog.c.build()?;
