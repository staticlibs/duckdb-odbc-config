...
pub(super) struct LoadDsnsDialogNui {
    inner: Rc<RefCell<LoadDsnsDialog>>,
    inner_events: Rc<LoadDsnsDialogEvents>,
    default_handler: RefCell<Option<nwg::EventHandler>>
}

impl LoadDsnsDialogNui {
    pub(super) fn result(&mut self) -> LoadDsnsDialogResult {
        self.inner.borrow_mut().result()
    }
}

impl nwg::NativeUi<LoadDsnsDialogNui> for LoadDsnsDialog {
    fn build_ui(mut dialog: LoadDsnsDialog) -> Result<LoadDsnsDialogNui, nwg::NwgError> {
        let mut events: LoadDsnsDialogEvents = Default::default();
        dialog.c.build()?;
...
impl Drop for LoadDsnsDialogNui {
    fn drop(&mut self) {
